...
## 2026-08-25 — Documentation pass (docs agent, tests/locales/watchdog tracking docs only)

Closing one specific loop this log itself flagged as in-flight, rather than opening a new full sweep — scope here is limited to re-verifying the one row below against the current tree.

- **PropDragging reachability row (Pass #7's Item 3 table, further corrected by the later appended note): CLOSED, verified directly against `client/radial.lua` and `client/combat.lua` at the current tree, not from the prior entries' word.** Pass #7 originally recorded PropDragging as implemented-but-unreachable ("`RequestDrag`/`ReleaseDrag`/`IsDragEngaged` have zero callers anywhere at `HEAD`"), then noted in the same entry that an uncommitted `client/radial.lua` diff already in flight would close that gap, and flagged it so a future pass wouldn't rediscover it as new. This is that pass: `client/radial.lua` now has a real, committed "Drag / Release" `k9_drag` item (`if Config.Features.PropDragging then ... end` block) whose `onSelect` calls `IsDragEngaged()` to branch, then `ReleaseDrag()` or `RequestDrag()` — the exact toggle shape Bite & Hold/Leash already use, gated the same way, not gated on `CanShowK9UI()` for the release branch for the same documented "no unbounded trap" reason as every other release-shaped action in that file. `client/combat.lua` still defines all three functions (`RequestDrag`, `ReleaseDrag`, `IsDragEngaged`) exactly as before. PropDragging is therefore reachable in-game today (still gated off by its own `Config.Features.PropDragging = false` default, per every other Phase-3 feature's standing convention) — no longer an open item.
- No other files were re-swept this pass; this entry does not supersede or re-verify anything else in this log (bark-audio, dependency versions, or any other carried-forward item above remain whatever the most recent entry that touched them says).

## 2026-08-25 (mid-morning) — Watchdog pass (scheduled trigger, project-lead recurring mode)

Clean overall. One real cross-file wiring gap found and flagged (new audio asset not yet in the manifest), one in-flight spec file correctly identified as the sole source of test-suite red, no regressions in the five standing checks, dependency maintenance status re-confirmed with no drift.

**Unreviewed commits.** No "Watchdog:" marker since `d8dac12` (last commit this log's own history touches). 15 commits landed since then, through `ce4d776` (HEAD at pass start): `9808e56, 885197a, e9949aa, 5f94da4, 10d48fd, b6b8233, 4d64ce7, 9b92f3a, 160058d, ee11929, 15797b1, 21680c4, 8ddf51b, 9e6ee6d, ce4d776`. All reviewed now, mostly test-coverage/locale-migration/decision-triage work; two carried real code changes, both verified directly against source rather than trusted from the commit message:
- `9808e56` — a genuine new security fix, confirmed real: `server/propattachment.lua`'s `confirmPropAttached` handler now has a `FindOtherPropAttachmentByNetId(netId, citizenid)` guard (line 512) placed after every other rejection branch, rejecting a confirm that names a netId another citizen already owns — closes the exact cross-citizen collision `server/fetch.lua` already defended against. Locale key `propattachment.attach_failed_already_tracked` exists in `locales/en.json` and is called correctly.
- `ee11929` — confirmed real: both `job.grade.level` comparisons in `server/certifications.lua` (`IsEligibleCertifier` line 427, the `autoAccessGrade` bypass line 337) now explicitly require `type(job.grade.level) == 'number'` before comparing, closing an uncaught-error path on a present-but-non-numeric grade level from `qbx_core`.
- `ce4d776` (landed mid-pass, after the above 14) — a decision-closure commit for `REFACTOR_ROADMAP_2.md` item 4 (tenure cache query cost), reasoned from measured load bounds rather than another rewrite; matches the now-passing `tenure_spec.lua` case it renamed from DISCREPANCY to closed. No concerns.
- The rest (spec-coverage additions across combat/wellbeing/inventory/fetch/propattachment/main, the `DECISIONS_NEEDED.md` triage, the medkit double-toast fix, four locale-migration commits reaching 305/306 keys, the audio-sourcing decision doc) were spot-read against their diffs and match their messages; nothing landed silently.

**Dependency maintenance status — re-verified, no drift.** Fetched `fxmanifest.lua`/`package.json` directly from `raw.githubusercontent.com` (reachable) rather than trusting the prior entry: `overextended/ox_lib` = `3.39.0`, `overextended/ox_target` = `1.18.1`, `overextended/oxmysql` (via `package.json`, this resource ships no root `fxmanifest.lua`) = `2.14.1` — all three exact matches to the last-recorded figures, no version drift. Confirmed via `WebFetch` on the live GitHub pages that `overextended/ox_lib`, `overextended/ox_target` and `overextended/oxmysql` show **no** archive banner and have recent open issues/PRs (active). Separately confirmed `CommunityOx/ox_lib`'s page **does** show "This repository was archived by the owner on Apr 28, 2026. It is now read-only." One wrinkle chased down and resolved, worth recording so a future pass doesn't re-trip on it: `CommunityOx`'s own `.github` profile README (fetched raw) describes itself as founded "after the original Overextended team archived their work" — read in isolation that sentence sounds like it contradicts "Overextended is canonical," but that text describes CommunityOx's founding rationale from earlier in 2026, not current state; CommunityOx was itself subsequently archived April 28, 2026 (per a live GitHub archive banner, corroborated by search results), and Overextended's own repos are demonstrably active and ahead in version (`ox_lib` 3.39.0 vs. the dead fork's stalled `3.32.3`). Net: standing conclusion unchanged, now re-confirmed from primary sources this pass rather than carried forward on the previous pass's word alone.

**Bark-audio gap (SPEC.md §7) — status changed, verified against the live tree, not assumed.** `html/sounds/bark.ogg` now exists (currently **uncommitted**, untracked in `git status`): confirmed a real Ogg Vorbis file via `file` (`Ogg data, Vorbis audio, mono, 44100 Hz`), 6,951 bytes, 0.35s. `html/sounds/CREDITS.md` (also uncommitted, modified) documents the full sourcing chain — downloaded from a live OpenGameArt page, trimmed from a 4-bark take to one bark, encoded to Vorbis, verified `OggS` magic bytes and `ffprobe` output, and credited under OGA-BY 3.0 (attribution-only, which the task's own note confirms was always sufficient — the file also records a further nuance that the OGA page's License(s) field lists CC0 as a second option alongside OGA-BY, but ships credited under OGA-BY regardless since that alone clears the bar without relying on the ambiguous reading). This is real, licensed, verified work, not a placeholder.
**However — a real gap this pass found, not fixed (out of my read-only scope): `fxmanifest.lua`'s `files{}` block does NOT list `html/sounds/bark.ogg`** (verified directly — only `html/index.html`, `html/style.css`, `html/app.js`, `locales/en.json` are listed at lines 54-68). `CREDITS.md`'s own new section already discloses this exact gap under "Outstanding follow-up." Until that one line is added, the file will not actually ship to clients despite existing on disk and being correctly licensed — needs a one-line `fxmanifest.lua` edit from whoever owns that file next. The other four required sounds (`bark_alert.ogg`, `bark_aggressive.ogg`, `bark_calm.ogg`, `growl_ambient.ogg`) are still absent, deliberately deferred per `AUDIO_SOURCING.md` since all their gating features ship `false` by default.

**Five regression spot-checks — all pass, verified by reading the surrounding function body, not by grep count alone:**
- `AgilityBasicJump` — real file-scope gate, `client/movement.lua:1116`: `if not Config.Features.AgilityBasicJump then` wraps a `CreateThread` that suppresses jump/duck controls only while flagged off; default (`true`) starts no thread.
- Role-aware `LeashPairs` — `server/main.lua:798-799` still writes `{ partner = ..., isK9 = true/false }` for both directions; `isK9` is read by the two independent teardown paths as before.
- `RevokeCertificationOffline` still calls `RefreshCertificationCache` — confirmed within the same function body, `server/certifications.lua:905` (function start) to `:987` (the call), no intervening function boundary.
- `client/vehicle.lua`'s `onResourceStop` cleanup — line 216, checks `resourceName`/`vehicleState`, calls `ReleasePedFromVehicleState` and nils `vehicleState`, unchanged.
- `client/radial.lua`'s `lib.registerRadial`/`lib.addRadialItem` split — lines 1158-1179: submenu registered under `id = 'k9unit'` via `lib.registerRadial` first, then exactly one opener added via `lib.addRadialItem` with `menu = 'k9unit'` pointing at it. Correct shape preserved.

**SPEC.md audit.** No live "Done"/"Resolved" claim found contradicted by code. Specifically checked: the scent-tracking item (§9 item 17) is a layered, self-updating entry (three dated updates in sequence) whose latest state — implementation written, flag still `false`, one disclosed unverified assumption about `ox_inventory`'s `swapItems` hook shape — matches `server/tracking.lua` exactly as read. §7's bark-audio framing ("small, easy-to-source asset requirement... not zero-asset") is still accurate even with `bark.ogg` now sourced, since 4 of 5 required files remain unsourced and the one sourced file isn't wired into the manifest yet. Note for future passes: §6.x's `- [ ]` checkboxes (74 of them, zero `- [x]` anywhere in the file) are the verbatim original feature-request text, never used as a live completion tracker in this document — confirmed by the total absence of any checked box despite most of §6 being long since implemented; don't mistake these for a stale-status report. Separately (not SPEC.md, but the same class of drift): `fxmanifest.lua`'s own comment above the `files{}` `locales/en.json` entry still says "2 of ~48 files are migrated, so this is a reference pattern, not a finished migration" — badly stale now that this session's locale work reached 306 keys across nearly every file; flagging alongside the manifest gap above since both land in the same file and same next edit.

**Health baseline.**
- `luac5.4 -p` over every `.lua` file: 69 files at pass end (68 at pass start; `tests/recall_spec.lua` appeared mid-pass, untracked) — **all parse, zero syntax errors**, including the in-flight file.
- `luacheck` from repo root: **0 warnings / 0 errors** across the 67 files that existed at check time (before `recall_spec.lua` landed).
- `qbx_k9unit/tests/run.sh`: **one file failed** — `tests/recall_spec.lua`, 1 of 27 assertions ("SOURCE AUDIT: the literal string 'CanShowK9UI' does not appear anywhere in server/recall.lua" — expected false, got true). Confirmed via `git status` that this spec file is **untracked** (mid-write, not yet committed) — it is not one of the 17 committed spec files. Confirmed the failure is a test-authoring artifact, not a code regression: the assertion does a bare substring search, and `server/recall.lua`'s own comments (lines 36, 49, 187) legitimately discuss "CanShowK9UI" in prose while documenting that the file deliberately never calls it — the string search can't distinguish a comment from a call site. Re-ran every git-tracked spec file individually (`lua5.4 <file>` per committed file, without touching the working tree): **all 17 pass, 720 cases, 0 failures** (admin_spec.lua and tests/admin_spec.lua's own working-tree diff — uncommitted, +357 lines of new coverage for `/k9auditxp`/`/k9auditdept` — also passes at 50 cases including that in-flight addition). `html/sounds/CREDITS.md` and `tests/admin_spec.lua` are the two other files currently modified-but-uncommitted, alongside the two untracked new files (`bark.ogg`, `recall_spec.lua`) — all four are live in-progress work from other agents, not artifacts of this pass.

### For the invoking session to route
1. One-line `fxmanifest.lua` fix: add `'html/sounds/bark.ogg'` to the `files{}` block (line ~54-68) so the newly-sourced, correctly-licensed bark asset actually reaches clients. Same file, same edit: update or remove the now-stale "2 of ~48 files migrated" locale comment a few lines above it.
2. No action needed on `tests/recall_spec.lua` beyond letting its own author finish it — the one failing assertion is a substring-match false positive against the file's own explanatory comments, not a `server/recall.lua` defect; flagging here only so nobody mistakes today's red `run.sh` output for a regression.
3. Nothing else outstanding from this pass; carried-forward items not touched here (the 4 remaining bark-adjacent sound files, any open `DECISIONS_NEEDED.md` items) are unchanged from whatever the most recent entry touching them says.

**2026-08-25 (mid-morning) — Pass covered: 15 commits reviewed (`d8dac12`..`ce4d776`), dependency maintenance re-verified against live sources (no drift), bark-audio gap re-checked (asset now sourced but not yet manifest-wired), five regressions re-verified by direct read, SPEC.md audited (no stale Done/Resolved claim found; one stale comment found in fxmanifest.lua instead), full luac5.4/luacheck/run.sh baseline run (clean except one confirmed in-flight, untracked spec file).**

---

**2026-08-25 (12:46 UTC watchdog pass) — 8 commits reviewed (`c885f72`..`5a6f64c`), one MAJOR external change found, five regressions re-verified clean, full baseline run.**

- **CORRECTED, SAME DAY — the entry below was WRONG in its conclusion. Read this first.** Verification against primary sources found the real picture: Overextended went dark for about a year (April 2025 to April 2026), CommunityOx forked to cover that gap, and then **Overextended's own repositories came back** in late April 2026, within days of CommunityOx winding itself down. The four repos this resource actually names in `fxmanifest.lua` are `overextended/ox_lib`, `overextended/ox_target`, `overextended/oxmysql` and `overextended/ox_inventory`, and **none of them is archived.** `overextended/ox_lib` was re-checked directly: no archive banner, 5 open issues, 2 open pull requests, last commit eight days ago; ox_inventory issues are being triaged within the last fortnight. The successor is not a new fork — it is the original project, active again. Versions cited here match current releases exactly, with no lag: ox_lib 3.39.0, ox_target 1.18.1, oxmysql 2.14.1, ox_inventory 2.47.9.

  **The lesson is the more useful half.** I checked two pages, found two archive banners, and generalised from them. The archived organisation was real; the inference drawn from it was not. GitHub's rendered pages also returned demonstrably wrong dates when summarised — a `2026-07-13` release came back as "July 13, 2024" — and only the repos' `.atom` feeds gave correct timestamps. **For any future dependency check: read the repo the manifest actually names, never a fork of it, and take dates from the atom feed, not the rendered page.**

  **What survives the correction, and is genuinely worth knowing:** this exact dependency set DID go dark for a year, on live production infrastructure, within the last five months. `fxmanifest.lua`'s hard `dependencies` block means the server will not start without all four, and `shared/compat/` does NOT insulate against that — it abstracts inventory, target, framework, dispatch and ambulance, but not ox_lib (607 `locale()` call sites, 179 `lib.callback` sites) and not oxmysql (direct `MySQL.*`). The mitigation is operational, not code: keep a saved local copy of the current working versions. **Reassess only if an archive banner reappears on one of the four, or six months pass with no commits on any of them.** Nothing to do before then.

- **SUPERSEDED, kept rather than deleted so the reasoning stays auditable — the original entry, whose conclusion was wrong:**
  - **NEW, MAJOR — the ox dependency line is now unmaintained end to end.** Previous passes recorded ox_lib/ox_target/ox_inventory/oxmysql as maintained under CommunityOx after Overextended archived theirs. That is no longer true: `CommunityOx/ox_lib` reads "archived by the owner on Apr 28, 2026", and the **CommunityOx organization itself** reads "marked as archived by an administrator on Apr 28, 2026. It is no longer maintained" — with ox_lib, ox_target, ox_inventory, oxmysql and ox_core all showing as Public archive. All four are HARD `dependencies` in `fxmanifest.lua`; FXServer will not start this resource without them. Dispatched to tech-scout to establish whether a maintained successor exists and to produce one plain-language recommendation for the owner. **Do not treat this as settled until that report lands** — two web pages is a thin basis for a decision this size, and the next pass should re-check rather than inherit this entry.
- **Bark-audio placeholder gap (SPEC.md §7) — CLOSED.** All five sounds now ship AND are listed in `fxmanifest.lua`'s `files{}` block: bark.ogg, bark_aggressive.ogg, bark_alert.ogg, bark_calm.ogg, growl_ambient.ogg. The previous pass's outstanding action item (wire bark.ogg into the manifest) is done. Provenance and licensing are recorded in `html/sounds/CREDITS.md`.
- **Five regression spot-checks, all PASS, verified by reading code and not comments:** `Config.Features.AgilityBasicJump` is read at client/movement.lua:1225; `LeashPairs` is still role-aware with `isK9` set in both directions at server/main.lua:834-835; `RevokeCertificationOffline` still calls `RefreshCertificationCache`; client/vehicle.lua:252 still has its real `onResourceStop` cleanup handler; client/radial.lua still calls `lib.registerRadial` (4 sites) alongside `lib.addRadialItem` — the 2026-08-23 hard-error fix is intact.
- **SPEC.md audit — no stale Done/Resolved claim found.** The one that looked stale was the thermal/night-vision resolution ("gates on `IsOwnModelK9()`, not `CanShowK9UI()`"), which read as broken after today's role/model decoupling. It is not: `IsOwnModelK9()` was itself deliberately widened to answer role-OR-model (client/main.lua), so vision works on any ped and the resolution's original reasoning — "the K9's own innate perception, available to anyone playing a K9 character" — is preserved rather than contradicted. Recorded here because it is exactly the kind of claim a future pass will re-flag.
- **Baseline:** all 143 .lua files parse clean under `luac5.4 -p`; `luacheck` 0 errors; locale cross-check passes at 442 keys / 668 call sites, 0 missing, 0 unused; `tests/run.sh` green except spec files belonging to features being written concurrently at the moment of the run.
- **Correction to my own work this pass, recorded so it is not repeated:** I applied a PARTIAL fix to `server/sarcalls.lua`'s missing feature gate — I wrapped only the config asserts, leaving `NewCooldown`, the tick thread and both event registrations running unconditionally with the feature off. That file's author caught it and replaced it with the standard full top-level early-return plus a regression test. Gating a symptom is not gating the feature.
