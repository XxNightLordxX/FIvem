...
t.test('a server-side denial with reason = too_far shows the SPECIFIC too_far notice, not the generic action_denied one', function()
    local f = newTrainingFixture()
    f.fireTrainingModeChanged(true)
    f.setCallbackResponse({ ok = false, reason = 'too_far' })
    f.commands.k9trainsearch()
    local notif = f.lastNotify()
    t.equals(notif.type, 'error')
    t.equals(notif.description, f.env.locale('training.too_far'), 'too_far must tell the player to get INTO a training zone, not to "wait" -- waiting is actively wrong advice here')
end)

t.test('a server-side denial with reason = no_access shows the SPECIFIC no_access notice, not the generic action_denied one', function()
    local f = newTrainingFixture()
    f.fireTrainingModeChanged(true)
    f.setCallbackResponse({ ok = false, reason = 'no_access' })
    f.commands.k9trainsearch()
    local notif = f.lastNotify()
    t.equals(notif.type, 'error')
    t.equals(notif.description, f.env.locale('training.no_access'), 'no_access must tell the player they are not certified, not to "wait" -- waiting is actively wrong advice here')
end)

t.test('a server-side denial with reason = not_training shows the SPECIFIC not_training notice, not the generic action_denied one', function()
    local f = newTrainingFixture()
    f.fireTrainingModeChanged(true)
    f.setCallbackResponse({ ok = false, reason = 'not_training' })
    f.commands.k9trainsearch()
    local notif = f.lastNotify()
    t.equals(notif.type, 'error')
    t.equals(notif.description, f.env.locale('training.not_training'))
end)

t.test('a server-side denial with an unrecognized/missing reason falls back to the generic action_denied notice', function()
    local f = newTrainingFixture()
    f.fireTrainingModeChanged(true)
    f.setCallbackResponse({ ok = false, reason = 'some_future_reason_this_file_has_not_been_taught_yet' })
    f.commands.k9trainsearch()
    local notif = f.lastNotify()
    t.equals(notif.type, 'error')
    t.equals(notif.description, f.env.locale('training.action_denied'))
end)
