...
> **HISTORICAL DESIGN DOCUMENT.** This captures the plan and reasoning behind
> this resource's five build phases, not a live description of today's code —
> `config.lua` and the actual `.lua` files always win if something here
> disagrees with them. See `DOCS_INDEX.md` for where to look for current
> status instead (`README.md` for technical reference, `PROJECT_STATUS.md`
> for a plain-language snapshot and open decisions, `CHANGELOG.md` for what
> actually shipped and when).
>
> **Merged 2026-08-25** from four previously-separate files — this document,
> `PHASE3_SPEC.md`, `PHASE4_SPEC.md`, and `PHASE5_SPEC.md` — into one, per an
> explicit, repeated request to reduce the document count. Those three files
> now exist only as short redirect stubs. **Every section number below is
> unchanged from the pre-merge files** (§1–§11 are this document's own
> original numbering; §12 is former `PHASE3_SPEC.md` unchanged; §13 is former
> `PHASE4_SPEC.md` unchanged; §14 is former `PHASE5_SPEC.md` unchanged) —
> every `.lua`/`.md` citation of the form `PHASEn_SPEC.md §X.Y` in this
> codebase was repointed to `SPEC.md §X.Y` in the same pass, section number
> untouched. This merge also cut a large amount of superseded decision
> history, revision-by-revision narration, and meta-commentary that had
> accumulated across the four files — what remains is the decisions the code
> depends on, the constraints that would be re-broken if forgotten, and the
> questions still genuinely open. Where a rejected option is cut, a one-line
> note says it was rejected and why, so it isn't re-proposed from scratch.

Target stack: Qbox (qbx_core, ox_lib, ox_target, ox_inventory), oxmysql.
Originally a greenfield design (no prior K9/permission/certification code to
extend). All five phases below have since shipped real code — see
`README.md`/`CHANGELOG.md` for current status; this document is not updated
to track it.
...
- **True live-video PiP camera feed** of the dog's point of view. Concluded
  infeasible without new native support — see §7. Do not re-propose without
  a real engine-level capability landing in FiveM first; only a feasibility
  spike was ever committed, and it already ran (Phase 5).
...
Each group is tagged with the phase it ships in.
...
Superseded in full detail by §12 — the bullets below are the high-level
anchor:
- [ ] Bite-and-hold interrupts a target's sprint/weapon-fire until Recall or
      timeout (approximated via task/animation + control-disable, §7).
- [ ] Non-lethal takedown ragdolls a fleeing target without lethal damage
      (health floor enforced server-side).
- [ ] Handler-down defense pre-selects the nearest hostile when the
      handler's health drops below threshold, no manual radial input needed
      to reach the confirm step.
- [ ] Prop dragging moves a downed target at reduced speed toward a
      handler-designated point; releasing ends it.
- [ ] All four are independently toggleable with a cooldown each.

### 6.3 Scent & Advanced Tracking — Phase 2

Superseded in detail by §11.5 — kept here as the high-level anchor:
- [ ] Scent tracking reveals a client-side trail toward the nearest
      configured scent source within range.
- [ ] Blood trail tracking is identical, keyed to recent damage-event
      locations instead of item drops.
- [ ] Water tracking degrades/breaks the trail at a water crossing,
      requiring re-acquisition on the far bank.
- [ ] Gunpowder residue sniffing keys off recent weapon-discharge locations
      within a configurable window.
- [ ] Search vehicle/person reveals configured contraband if present via a
      sniff animation + ox_target zone.
- [ ] Contraband alert tiers vary the audio/animation response by total
      weight found (`Config.ContrabandAlertTiers`).
- [ ] Thermal/night vision use native toggles only — no custom shader work.

### 6.4 Vision & Tactical Systems — Phase 2 (basic) / Phase 5 (PiP spike)

Superseded in detail by §11.5 for the Phase 2 bullet.
- [ ] Thermal and night vision (see 6.3) ship Phase 2.
- [ ] K9 camera feed PiP is a Phase 5 **research spike only** (§7) — no PiP
      video feed is a committed deliverable.

### 6.5 Qbox Integration, Inventory & Progression — Phase 4
- [ ] Dog has an ox_inventory stash (`RegisterStash`, keyed to the K9
      player's own citizenid), opened via ox_target within a configurable
      range.
- [ ] Job/certification gating (§4) is the only gate on any K9 feature.
- [ ] XP accumulates from configured actions and persists per-K9-citizenid.
- [ ] Crossing an `Config.XPTiers` threshold applies `speedMultiplier` and
      `scentRange` immediately, no resource restart.
...
All five phases below have shipped; see `README.md`/`CHANGELOG.md` for
current state. Kept here only for the dependency reasoning, not as a live
task list.

### Phase 1 — vertical slice
Config framework; `k9_certifications` table (§4.3); certification
grant/revoke/check (§4); K9-model/access detection (§4.5); native quadruped
movement + camera toggle; leash; radial menu; vehicle entry/exit; basic
bark. **Definition of done:** a certified K9-model player in a configured
department can use the radial menu (leash, vehicle, bark); an uncertified
or out-of-department player cannot, even via a direct event call; getting
fired ends access immediately.

### Phase 2 — tracking & basic vision
Scent/blood/water/gunpowder tracking, search zones, contraband alert tiers,
door interaction, thermal/night vision. Full detail: §11.

### Phase 3 — combat & advanced agility
Bite-and-hold, non-lethal takedown, handler-down defense, prop dragging,
advanced agility. Full detail: §12.

### Phase 4 — inventory, progression, vitality
ox_inventory K9 stash, XP/progression, vitality subsystem, K9 medkit,
contraband screen effect. Full detail: §13.

### Phase 5 — audio/props polish + camera R&D spike
Advanced bark radial, proximity audio attenuation, prop attachments, fetch
mechanic, deployable kennel, camera feed feasibility spike (§7 — concluded
no PiP work is scheduled). Full detail: §14.
...
code comments (`SPEC.md §9 item N`). Resolved items are kept, condensed,
rather than removed or renumbered, so those citations keep resolving to
...
11. **ox_inventory export names/signatures** for search/contraband reading
    ...
    (`phase2_notes/RESEARCH_ARCHIVE.md#contraband-search`) and for scent
    (item 17 below); K9 Inventory/K9 Medkit's own export shapes (§13.4.2,
    §13.4.4) were flagged with the same caveat when written — verify
    against the live `overextended/ox_inventory` source before trusting an
    unconfirmed export shape in this document.
...
17. **Scent tracking's server-side source resolution — was explicitly
    ...
    test against a live `ox_inventory` install — that one-time dev
    verification (log `json.encode(payload)` once against the target
    server's real version) has not been performed. `Config.Features.ScentTracking`
    ships `false` for this reason; do not enable on a live server until that
    verification runs. See `CHANGELOG.md` Known Limitations.
...
2. `qbx_k9unit:server:searchTarget(targetType: 'vehicle'|'person', targetNetId)`
   ...
   has defined feedback. Full export-signature detail in
...
- **Search vehicle/person contraband reading** — unambiguously possible;
  exact export names confirmed in `phase2_notes/RESEARCH_ARCHIVE.md#contraband-search`.
...
the certification grant path, not less.
...
rejected server-side. **Genuine cross-phase dependency:** whichever of
Phase 3/Phase 4 is implemented second must confirm this contract against
the other phase's actual shipped code.
...
issuable by a partnered handler on someone else's K9.
...
Server-authority, mirroring `phase2_notes/RESEARCH_ARCHIVE.md#contraband-search`'s
established pattern: (1) feature-flag + eligibility first, (2) **live
proximity check before any state mutation**, never a client-claimed
distance — closes the single most important "map-wide oracle" risk that
note flags, applied here to a heal effect instead of an inventory read, (3)
target-type re-verified server-side via `GetEntityModel`, (4) item
consumption is server-authoritative via ox_inventory (exact
export/registration pattern not independently verified this session — same
caveat class as §9 item 11), (5) cooldown stamped **before** the heal
completes, not after, (6) restoration clamped to each value's own max,
never allowed to overheal.
...
- **Cross-phase dependency:** `server/combat.lua` must call
  ...
  its own request-validation order — whichever phase's code lands second
  should confirm this contract against the other phase's actual shipped
  code.
- **Shared-infrastructure requirement:** `RecomputeK9MoveRate()` (Decision
  ...
  speed-affecting features ship together — whichever phase's coder gets
  there first should build it as shared infrastructure, not a
...
- Every ox_inventory export/registration pattern this phase relies on
  ...
  any whitelist-enforcement hook) is unverified against real source or a
  live install — recommend a dedicated verification pass before
  `client/inventory.lua`/`server/inventory.lua`/`client/medkit.lua`/
  `server/medkit.lua` implementation, mirroring that note's methodology.
...
would only ever apply while a Phase 2 tracking flag is also on — dressing
that narrower thing up as satisfying the original "hidden suspect" wording
would be exactly the kind of undisclosed scope-narrowing §12.0 item 7
already rejected for `LeashPairs` reuse. A future pass remains free to wire
this in explicitly via the hook below if judged acceptable later.
...
concept considers relevant, or `nil`. Unlike §12.0 items 5/6 (which at
least have a plausible `metadata.*` guess), there is no ecosystem-dominant
...
inert — the identical "plumbing real, payload absent, safe no-op" posture
`BasicBarkSounds`' placeholder soundset convention already established.
Flipping the feature flag on without ever wiring the hook is not a bug.
...
|---|---|---|
| `ProximityAudioFX` | Audio mechanism: yes (same safe placeholder-soundset convention as `BasicBarkSounds`). Trigger condition: no — inert until `SuspectDistanceSource` is wired (fork 2). | Real growl/pant audio (same sourcing task already named for `AdvancedBarkRadial`), and a `SuspectDistanceSource` implementation. |
| `PropAttachments` | No — unlike `DeployableKennel`, no research pass found any generically-real placeholder prop for a quadruped vest/harness. This document does not invent one. | A real prop model name — `propModel` ships `nil`, the feature safely no-ops (one log line) until an operator sets it. |
| `FetchMechanic` | Yes, for the ball itself — `prop_tennis_ball` is the highest-confidence prop name found, traced to a real shipped community script's own default. Mouth-*carry* fidelity is conditional on fork 1. | Nothing required for a working feature. Optionally a custom "evidence bag" prop reskin (not required for v1). |

### Fork 4 (smaller, load-bearing for §14.2/§14.3) — where does `ProximityAudioFX`'s automatic growl-relay cooldown live?
...
| 5b — independent, parallel with 5a | `ProximityAudioFX` | Unrelated to bone attachment; ships inert until `SuspectDistanceSource` is wired (not part of this sub-phase's completion criteria). |
| 5c — depends on 5a | `PropAttachments` | The attach mechanism needs zero new research (already proven, `client/vehicle.lua`); blocked only on 5a's recorded index existing — a `nil` value degrades to the documented fallback offset, so this is quality-blocked, not fully blocked. |
| 5d — spawn/throw/lifecycle independent of 5a; mouth-carry depends on 5a | `FetchMechanic` | The ball's lifecycle can start immediately with `mouthCarryModeByModel` defaulted to `'fake'`; upgrading a model to `'attach'` is a narrow, non-blocking-to-the-rest-of-the-feature dependency. |
...
-- Config.K9BoneIndices entry above is still nil. Untuned, reasonable-looking
-- guesses, mirroring client/vehicle.lua's own root-bone-plus-offset precedent.
...
Config.ProximityAudioFX = {
    ...
    distanceTiers = { -- ordered nearest-to-farthest; opaque placeholder-audio growlType strings, same posture as AdvancedBarkRadial's `sound` field
    ...
    serverCooldownMs = 500, -- own cooldown, independent of BarkCooldown -- fork 4. Must stay below the fastest tier's cadenceMs.
}
...
Config.PropAttachments = {
    ...
    -- Additional slots (harness, tracking-camera housing) follow the same
    -- shape -- add entries, not new Lua branches. A "tracking camera" slot
    -- is COSMETIC ONLY -- no functional link to CameraFeedPiP.
}

Config.FetchMechanic = {
    ballPropModel = 'prop_tennis_ball', -- confirmed real & free, highest-confidence name found
    throwForwardOffsetMeters = 1.0, throwUpOffsetMeters = 1.2,
    throwForceForward = 12.0, throwForceUp = 6.0, -- untuned placeholders, shape (not exact values) matches one real community precedent
    throwCooldownMs = 5000, pendingThrowTtlMs = 15000,
    maxBallLifetimeMs = 300000, -- safety-valve force-despawn, mirrors this resource's "no unbounded state" discipline (LEASH_REQUEST_TTL_MS, pendingPlacementTtlMs)
    pickupInteractDistanceMeters = 2.0,
    -- Per-model mouth-carry mode -- fork 1. 'attach' only valid once a
    -- model's mouthBoneIndex is resolved AND its clipping check passed.
    -- ALL FOUR default to 'fake' (delete-and-animate) until explicitly
    -- upgraded -- the safe, zero-bone-dependency default.
...
| `client/proximityaudio.lua` | New | `ProximityAudioFX`'s tick loop (gated at file-load on both `ProximityAudioFX` and `BasicBarkSounds`), evaluating `SuspectDistanceSource()`, picking a tier, sending `relayGrowl`; also the receiving `playGrowl` handler via the existing `PlaySoundOnNetworkEntity` helper. |
| `server/main.lua` | Extends | `relayGrowl` handler — a near-identical sibling of `relayBark`, but with its **own** cooldown table/length cap (fork 4). |
| `client/main.lua` | Extends (small) | New exported `CanActAsK9Handler() -> boolean` (`HasK9Access()` directly, no `IsOwnModelK9()` requirement) — needed because `FetchMechanic`'s "Throw" is a **human handler** action per the original "on a handler command" wording, not necessarily a K9-riding player. |
...
| `client/radial.lua` | Extends (small) | An "Equipment" submenu opener built from `Config.PropAttachments`, and a "Throw Fetch Ball" item gated via `CanActAsK9Handler()`, not `CanShowK9UI()` — the one deliberate departure from this file's otherwise-universal gate. |
| `client/fetch.lua` / `server/fetch.lua` | New pair | `FetchMechanic`'s full lifecycle, directly modeled on `client/kennel.lua`/`server/kennel.lua`'s shipped spawn/confirm/cleanup pattern. **One deliberate divergence, disclosed:** the confirm step does not tightly re-validate the reported position against the server-chosen spawn point the way kennel placement does — a thrown, physics-simulated ball's landing spot is a low-stakes, client-simulation-owned cosmetic fact with no server-authoritative consequence riding on it. |
...
— deliberately the same "broadcast wide, let native audio 3D-cull" posture
as `relayBark`/`relayDoorScratch`, **not** the distance-filtered posture
`server/search.lua`'s alert broadcast uses (a growl reveals no sensitive
per-player information).

Acceptance:
- [ ] Defaults `false`; the tick loop registers nothing unless both
      `ProximityAudioFX` and `BasicBarkSounds` are true (checked once at
      file-load).
- [ ] With `SuspectDistanceSource` left `nil`, the feature is observably
      inert — no growl, no error, no console spam.
- [ ] With the hook wired, a growl fires at the matching tier's own
      cadence, audible to nearby players via native distance falloff.
- [ ] `relayGrowl` independently re-validates the flag, access, `growlType`
      length, and its own `GrowlCooldown` — none shared with `relayBark`'s
      equivalents.
- [ ] Manual "Bark" and the automatic growl loop never cross-throttle each
      other.
- [ ] No server-authoritative consequence is ever gated on this feature
      firing.

Open: the real "hidden suspect" detection primitive itself (explicitly
deferred, fork 2); real growl/pant audio sourcing (same task already named
for `AdvancedBarkRadial`).

### 14.4.2 Prop Attachments (`Config.Features.PropAttachments`)

A config-driven list of "slots," each its own context-sensitive
Equip/Unequip radial item under a new "Equipment" submenu. **Self-administered,
...
precedent (attaching something to the acting player's own already-controlled
ped, purely client-side, zero server round trip) — deliberately **not**
`DeployableKennel`'s pattern (full server-computed-coords + registry),
because a kennel is a freestanding world object that can outlive its
placer's session; a worn prop has no independent existence, it lives and
dies exactly with the K9's own ped.
...
configured" and does nothing (one console breadcrumb, never silent). On
Unequip: `DetachEntity` + `DeleteEntity`, also on model swap, K9 vehicle
entry/exit, disconnect, and `onResourceStop`. **Disclosed limitation:** no
server-side re-check forces an unequip on later decertification — mirrors
`client/vehicle.lua`'s own existing "no forced eject" posture.

Acceptance:
- [ ] Defaults `false`.
- [ ] Each slot appears gated by `CanShowK9UI()` under the Equipment
      submenu.
- [ ] Equipping a `nil`-`propModel` slot notifies and does nothing
      observable — no error, no crash.
- [ ] A resolved bone index attaches exactly there; an unresolved one falls
      back to root+offset — never a crash or an unexplained silent
      mis-position.
- [ ] Every automatic cleanup trigger leaves no orphaned prop entity.
- [ ] At most one attached prop per slot per K9 — re-selecting toggles off,
      never duplicates.
- [ ] No server event of any kind is registered for this feature.
- [ ] No "tracking camera" slot copy implies live video/feed functionality.

Open: the real prop model(s) (fork 3, operator-supplied, no safe generic
placeholder found); whether nested attachment (prop on a K9 that then
enters a vehicle) renders correctly — untested by any research pass;
whether a bone index needs an additional rotation/offset tuning pass beyond
the raw index — deferred to the sweep session itself.
...
vector from the thrower's own live position/forward vector, and instructs
that same client to create and throw the object (server picks the numbers,
the client's own physics engine simulates the resulting arc — never
re-verified server-side, since nothing server-authoritative depends on
exactly where the ball lands). **Pursue:** zero scripting — the K9 player
walks to the visible ball under completely normal native locomotion; no
autonomous pursuit task exists. **Pick up:** within
`pickupInteractDistanceMeters`, an ox_target option gated by
`CanShowK9UI()`, server re-validated via `IsConfiguredK9Model` +
`HasK9Access`. **Carry:** branches on
`mouthCarryModeByModel[currentModel]` — `'attach'` (only for a model whose
sweep found a usable index and passed the clipping check) does a one-shot
`AttachEntityToEntity` at the mouth bone (deliberately **not**
re-asserted every tick, unlike `PropDragging` — there is no adversarial
party working against a K9 carrying its own fetched ball, so item 8's
every-tick discipline doesn't apply here); `'fake'` (the default for every
model) deletes the world ball and plays a carry animation/scenario instead
(exact animation choice is a genuinely open question — no confirmed
"carrying something in its mouth" vanilla clip was found). **Drop:** a
second radial toggle releases wherever the K9 stands — no "return to
handler" objective of any kind. **Lifecycle:** `maxBallLifetimeMs`
force-despawns an un-dropped ball and frees the thrower's slot;
`playerDropped`/`onResourceStop` cleanup mirrors `server/kennel.lua`'s
two-handler pattern, adapted for "thrower" and "carrier" both being
possible owning roles.

Acceptance:
- [ ] Defaults `false`.
- [ ] "Throw Fetch Ball" is available to any player passing
      `CanActAsK9Handler()`, independent of currently riding a K9 model.
- [ ] A thrown ball is a real, physics-simulated, visible object, zero
      additional assets required.
- [ ] A K9 player reaches the ball using nothing but native locomotion — no
      scripted pursuit task exists.
- [ ] "Pick Up Ball" only appears/succeeds for a player passing
      `CanShowK9UI()`, server re-verified.
- [ ] `'attach'` mode visibly follows the mouth region; `'fake'` mode
      deletes the object and shows an animation — never ambiguous or mixed
      mid-carry.
- [ ] Dropping releases at the K9's current position in either mode.
- [ ] At most one active ball per thrower; a second throw attempt is
      rejected, not queued or duplicated.
- [ ] An un-dropped ball is force-despawned after `maxBallLifetimeMs`,
      verified by direct test.
- [ ] Disconnect (thrower's or carrier's) and `onResourceStop` both leave no
      orphaned ball.
- [ ] `mouthCarryModeByModel` defaults every model to `'fake'` — no model
      ships in `'attach'` mode without an explicit, disclosed edit
      following a real sweep + clipping check.

Open: whether `'fake'` mode's animation should reuse the existing
`WORLD_DOG_BARKING_*` scenarios (untested for this intent) or needs a
different, unidentified clip; fork 1's sweep outcome per model; an
"evidence bag" prop reskin — not attempted, not required for v1.
...
- Config-validator pass required before any of these three flags default
  ...
  guess.
- Asset-pipeline coordination needed for two of three features before they
  ship a non-placeholder experience (fork 3) — neither gap is a new
  category, both trace to already-named asset needs (§7, §9 item 7).
- `SuspectDistanceSource` and `propModel` are both, deliberately,
  ...
  not partially-implemented features — mirrors §2's own non-goal posture
  applied to two new cases.
- Nothing here depends on any in-flight NUI audio bridge work —
  `ProximityAudioFX` is specced against the existing, proven
  `PlaySoundFromEntity` pipeline, reusing `relayBark`'s shape almost
  verbatim. A future NUI audio bridge remains a legitimate upgrade path for
  continuous volume scaling, not required or assumed here.

## 14.6 — Quick-reference: decisions

1. Fork 1 (bone-index sweep) — **resolved** as a concrete, bounded
   procedure, not a design decision left open.
2. Fork 2 (`ProximityAudioFX`'s hidden-suspect detection) — **resolved** as
   a deliberate deferral: the delivery mechanism ships real; the trigger
   ships as an inert, `nil`-by-default hook.
3. Fork 3 (asset dependency) — **resolved** as an explicit per-feature
   disclosure, not silently assumed either way.
4. Fork 4 (`ProximityAudioFX`'s cooldown) — **resolved**: a new, independent
   event/cooldown pair, not a reuse of `relayBark`'s.
5. `PropAttachments`' trust model — **resolved**: purely client-side, no
   server event, directly extending `client/vehicle.lua`'s precedent.
   Disclosed limitation: no forced unequip on later decertification.
6. `FetchMechanic`'s throw-gate — **resolved**: `CanActAsK9Handler()`,
   distinct from `CanShowK9UI()`, since a human handler is implied by the
   original wording — a deliberate, narrow departure from the otherwise-
   universal radial gate.
7. `FetchMechanic`'s mouth-carry fidelity — **resolved as a safe default**:
   ...
   upgrades a specific model — never the reverse, never a guess.
8. Real growl/pant/vest/harness/camera-housing audio and prop assets — all
   already-named, not-yet-resourced content-sourcing tasks (fork 3), not
   new asks.
9. Whether any `WORLD_DOG_BARKING_*` scenario is a visually acceptable
   stand-in for `'fake'`-mode carry — untested, flagged, not assumed either
   way.
10. The real "hidden suspect" detection primitive itself — explicitly out
    of scope (fork 2), a precondition for a future `ProximityAudioFX` v2.
